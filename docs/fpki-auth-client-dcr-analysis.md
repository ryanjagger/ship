...
> **Section removed 2026-05-21.** This section described `/api/federation`, `services/fpki.ts`, `services/credential-store.ts`, and `vendor/@fpki/auth-client` — none of which exist in this repo. The implementation either never landed or was removed. The rest of this document (overview, SDK exports, configuration requirements, security considerations) remains accurate as an analysis of the upstream SDK.
