...
router.get('/my-week', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    // Pre-compute the 7 standup dates so the standups query can run in
    // parallel with the others (it doesn't depend on personId, just userId
    // and the week dates from wave 1).
    ...
    // Wave 2: queries 3-7 are independent — each only needs personId/userId
    // and the pre-computed dates. Running them in parallel turns the
    // wall-clock cost from sum(q3..q7) into max(q3..q7).
    const [planResult, retroResult, prevRetroResult, standupsResult, allocationsResult] = await Promise.all([
      ...
      // 5. Previous week retro (skipped query if previousWeekNumber <= 0)
      previousWeekNumber > 0
        ...
        : Promise.resolve({ rows: [] as Array<{ id: string; title: string; properties: Record<string, unknown> }> }),
      // 6. Standups for the 7 days of the target week
      pool.query(
      ...
      // 7. Project allocations for the target week
      pool.query(
        `SELECT DISTINCT
           proj.id as project_id,
           proj.title as project_title,
