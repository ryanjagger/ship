...
export interface IEventBus {
  publish(client: PoolClient, events: ShipWebhookEvent[]): Promise<string[]>;
...
class InProcessEventBus implements IEventBus {
  async publish(client: PoolClient, events: ShipWebhookEvent[]): Promise<string[]> {
    const ids: string[] = [];
    for (const event of events) {
      ...
      // Fan out one delivery per active subscription whose event set matches.
      await client.query(
        ...
        [event.id, event.workspace_id, event.type]
      );
      ids.push(event.id);
    }
    return ids;
