...
| `pg` boundary typed in `projects.ts` (Phase 5, first route) | 390 warnings in `projects.ts` (mostly `no-unsafe-member-access` / `no-unsafe-assignment` from `row: any` propagation) | 9 warnings; one `extractProjectFromRow(row: any)` antipattern eliminated; per-query `Row` interfaces + `pool.query<Row>(...)` | pending |
...
- Phase 5 (`projects.ts`) changes are in the working tree, pending commit
