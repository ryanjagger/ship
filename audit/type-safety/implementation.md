...
| `req.userId!` / `req.workspaceId!` non-null assertions (Phase 2) | 236 occurrences across 20 route files | zero in production code; replaced with `assertAuthed` / `assertUserAuthed` narrowing helpers that actually check at runtime | pending |
...
- **3 commits** on `implement/type-safety` so far:
  - `e3a41e4` — lint baseline tooling (Phase 0)
  - `038addb` — Phase 1 quick wins
  - `264a929` — Phase 1 doc backfill
- Phase 2 changes are in the working tree, pending commit
