...
2. Added the `accessibility` prop to `<DndContext>` with two keys:
   ...
   - `announcements`: four callbacks (`onDragStart`, `onDragOver`, `onDragEnd`, `onDragCancel`) that return strings the dnd-kit live region announces. Helpers convert dnd-kit ids into human-readable text: `getIssueLabel(id)` (closes over the `issues` prop, returns `#${ticket_number}: ${title}` or falls back to the id), `getColumnLabel(id)` (looks up `COLUMNS.title`), and `describeOverColumn(overId)` (resolves an `over.id` to its destination-column label, see correction note below).

**Correction — over.id can be an issue, not a column.** The first pass passed `over.id` straight to `getColumnLabel`. In practice `over.id` is often *another issue's* id (when a card is hovered/dropped onto a sibling card rather than the empty column body); `getColumnLabel` then fell through to `String(id)`, so screen readers heard the raw issue id where the column name should have gone. The existing `handleDragEnd` (`web/src/components/KanbanBoard.tsx:101`) already had the right logic — "if `over.id` matches a column id use it, else find the issue with that id and use `issue.state`". That logic is now extracted into a `resolveColumnIdFromOver(overId)` helper used by both `handleDragEnd` and the `describeOverColumn` helper that the over/end announcements consume. If resolution fails (over isn't a column and isn't a known issue — defensive), the announcement falls back to the "no longer over a column" / "dropped outside any column" phrasing instead of leaking an id.

No new dependency. The implementation.md plan mentioned `@dnd-kit/accessibility`, which is the historical name; the canonical home in modern `@dnd-kit/core` ^6.x is the `accessibility` prop on `DndContext`, which is what we used. The existing `handleDragStart` / `handleDragEnd` handlers (which manage the `activeId` state for the `DragOverlay`) keep their behavior — `handleDragEnd` now uses the shared `resolveColumnIdFromOver` helper instead of its inline branching, so the announcement and the actual state update are derived from the same resolution.
