    ...
    <div className="flex items-center gap-3 flex-shrink-0">
      ...
      {showViewToggle && (
        <div className="flex rounded-md border border-border flex-shrink-0" role="group" aria-label="View mode">
          ...
          {viewModes.includes('related') && (
            <button
              onClick={() => onViewModeChange('related')}
              aria-label="Related view (AI grouped)"
              aria-pressed={viewMode === 'related'}
              className={cn(
                'px-3 py-1 text-sm transition-colors',
                viewMode === 'related' ? 'bg-border text-foreground' : 'text-muted hover:text-foreground'
              )}
            >
...
function RelatedIcon() {
  // Connected nodes — evokes "related / grouped".
  return (
    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 8a2 2 0 100-4 2 2 0 000 4zM7 20a2 2 0 100-4 2 2 0 000 4zM18 14a2 2 0 100-4 2 2 0 000 4zM8.7 7.3l7 4M15.7 13.3l-7 4" />
    </svg>
  );
}
