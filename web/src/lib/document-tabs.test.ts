...
describe('resolveTabLabels', () => {
  ...
  it('resolves dynamic labels with counts', () => {
    // Project's `issues` and program's `weeks` use dynamic label functions
    const projectResolved = resolveTabLabels(
      getTabsForDocumentType('project'),
      mockDocument,
      { issues: 5, weeks: 3 }
    );
    expect(projectResolved.find(t => t.id === 'issues')?.label).toBe('Issues (5)');
  ...
  it('resolves dynamic labels without counts', () => {
    const projectResolved = resolveTabLabels(getTabsForDocumentType('project'), mockDocument);
    expect(projectResolved.find(t => t.id === 'issues')?.label).toBe('Issues');
